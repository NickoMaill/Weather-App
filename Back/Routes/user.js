...
route.get("/login/:confirmationCode", async (req, res) => {
  ...
  const users = await Postgres.query(
    "SELECT * FROM users WHERE users.confirmation_code = $1",
    [req.params.id]
  );
  ...
  try {
    if (tokenDecode.exp - tokenDecode.iat < 600) {
      await Postgres.query(
        "UPDATE users SET is_active = true WHERE users.confirmation_code = $1",
        [req.params.confirmationCode]
      );
      mail.sendFinalEmail(
        tokenDecode.email,
        users.rows[0].first_name,
        users.rows[0].last_name,
      );
    }
  } catch (err) {
    console.error(err);
...
route.post(
  ...
  async (req, res) => {
    ...
    let user = await Postgres.query("SELECT * FROM users WHERE users.id=$1", [
      req.params.id,
    ]);
    let fileName = `${user.rows[0].surname}-${
      user.rows[0].surname
    }-${currentDate("file")}${type}`;

    let updateProfilePicture = await Postgres.query(
      "UPDATE users SET profile_picture = $1 WHERE user_id= $2",
      [`${req.file.destination}/${fileName}`, req.params.id]
    );
